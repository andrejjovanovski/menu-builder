...
export default function MenuBuilderPage() {
  ...
  const initialSettings = useMemo(() => {
    ...
    return {
      ...
      openBottomSheetOnClick: res.open_bottom_sheet_on_click ?? true,
      facebookUrl: res.facebook_url || "",
      instagramUrl: res.instagram_url || "",
      tiktokUrl: res.tiktok_url || "",
      phone: res.phone || "",
    };
  }, [selectedRestaurant]);
      ...
      <main className="flex-1 min-w-0 overflow-y-auto">
        {!selectedRestaurant ? (
          ...
          <div className="p-8 max-w-6xl mt-8 mx-auto space-y-8">
            {/* STICKY HEADER */}
            <header className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white p-6 rounded-[32px] border border-slate-200 shadow-sm md:sticky  top-0 z-10">
              <div>
                <a
                  href={`${window.location.origin}/${selectedRestaurant.slug}`}
                  target={"_blank"}
                  className="text-3xl font-black text-slate-900 tracking-tight hover:cursor-pointer"
                >
              ...
              <div className="flex flex-wrap items-center gap-3">
                <div className="flex items-center gap-2 w-full md:w-60 bg-slate-50 px-4 py-2.5 rounded-2xl border border-slate-200 focus-within:ring-2 focus-within:ring-indigo-500 transition-all">
